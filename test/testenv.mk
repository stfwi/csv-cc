...
TEST_SELECTION:=$(sort $(wildcard test/*$(TEST)*/test.cc))
TEST_BINARIES_SOURCES:=$(TEST_SELECTION)
...
$(BUILDDIR)/test/%/test$(BINARY_EXTENSION): test/%/test.cc test/testenv.hh
	@echo "[c++ ] $@"
	@mkdir -p $(dir $@)
	@cp -rf $(dir $<)/* $(dir $@)/
...
ifneq (,$(WITH_COVERAGE))
 ...
 .NOTPARALLEL:
 TESTOPTS:=--coverage -ftest-coverage -fprofile-arcs -fprofile-abs-path
endif

# Coverage main target (the one we invoke)
coverage:
	@$(MAKE) --jobs=1 coverage-runs WITH_COVERAGE=1 DEBUG=1
	@$(MAKE) coverage-files
...
$(BUILDDIR)/test/%/test.gcov: $(BUILDDIR)/test/%/test.log
	...
	@cd $(dir $<) && lcov -r lcov.info "*/test.cc" -o lcov.info >> lcov.log 2>&1
	@cd $(dir $<) && lcov -r lcov.info "*/microtest.hh" -o lcov.info >> lcov.log 2>&1
	@cd $(dir $<) && lcov -r lcov.info "*/testenv.hh" -o lcov.info >> lcov.log 2>&1
...
coverage-summary:
	...
	@cd $(LCOV_DATA_ROOT) && cat lcov.log | grep -iwP 'lines...' | grep -iwoP '([\d.]+)%' | sort | head -n1 | sed -e 's/%.*//g' >min-line-coverage
 ifeq ($(CI),1)
	@echo "[lcov] Coverage listing:"
	-@cd $(LCOV_DATA_ROOT) && cat lcov.log | sed 's/^/       /g'
	@echo "Checking if minimal line coverage > 90% ..."
	-@cd $(LCOV_DATA_ROOT) && test "$$(cat min-line-coverage | sed -e 's/\..*//g')" -ge 90
 endif
